...
PTE = THIS.parent / "decoder_xnn.pte"  # c_moe_exe/test/decoder_xnn.pte
...
# ensure eager custom ops are loaded (pytorch side)
importlib.import_module("deepseek_moe")  # provided by pyt_moe
...
from moe_wrapper import replace_layer_mlp_with_cpp  # use cpp gate+experts
...
# ---------- Always create exported_program for export_cpp comparison ----------
...
with torch.no_grad():
    ...
    # run the eager module reconstructed from export (same kernels as cpp/eager)
    y_export_cpp = exported_program.module()(x, cos, sin)

print("\n=== CPP / Python vs EXPORT_CPP ===")
diff_stats(y_cpp, y_export_cpp, "cpp    vs export_cpp")
diff_stats(y_python, y_export_cpp, "python vs export_cpp")

# ---------- (optional) rebuild .pte with this exported graph ----------
REBUILD = os.environ.get("REBUILD_PTE", "0") == "1"
if REBUILD:
    print("\n[rebuild] REBUILD_PTE=1 -> export .pte with current weights")
...
if not REBUILD:
    print(
        "\n[hint] not use REBUILD_PTE=1, so current .pte may be from previous random weights."
    )
    print(
        "       If you want strict consistency with tiny diffs, set: REBUILD_PTE=1 python run_pte.py"
    )
